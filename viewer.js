(function () {
  ...
  // Always go through the proxy to avoid CORS & to power the Download button
  const pdfUrl = pdfQ.startsWith("/api/proxy") ? pdfQ : `/api/proxy?url=${encodeURIComponent(pdfQ)}`;

  // toolbar links (open original if provided)
  ...
  // find StPageFlip regardless of export style
  const PageFlipLib = window.St?.PageFlip || window.PageFlip;
  if (!PageFlipLib) {
    document.getElementById("loading").textContent = "Flip engine failed to load.";
    console.error("StPageFlip not found on window (St.PageFlip or PageFlip).");
    return;
  }

  // controls
  let pageFlip, zoom=1, pdfDoc;
  ...
  fitSel.onchange = () => pageFlip?.update();
  ...
  async function boot(){
    try {
      ...
      // first two pages at good quality
      const targetW = document.getElementById("canvas").clientWidth / 2;
      const firstImg  = await renderPageToURL(pdfDoc, 1, targetW);
      const secondImg = pages >= 2 ? await renderPageToURL(pdfDoc, 2, targetW) : firstImg;

      // init flip
      pageFlip = new PageFlipLib(host, {
        width: 1100,
        height: 700,
        size: "stretch",
        maxShadowOpacity: 0.5,
        showCover: false,
        usePortrait: false,          // spreads like Issuu
      ...
      const initial = [firstImg];
      if (pages >= 2) initial.push(secondImg);
      const placeholders = Array.from({length: Math.max(0, pages - initial.length)}, () => firstImg);
      const all = initial.concat(placeholders);
      pageFlip.loadFromImages(all);

      loading.classList.add("hide");
      curEl.textContent = "1"; rangeEl.value = "1";
      ...
      // progressively replace placeholders
      for (let i = initial.length + 1; i <= pages; i++){
        const img = await renderPageToURL(pdfDoc, i, targetW);
